...
async def verify_staff_token(authorization: str | None = Header(None)):
    ...
    if not authorization or not authorization.startswith("Bearer "):
        raise AuthenticationError("Unauthorized access. Invalid or missing bearer token.")
    token = authorization.split(" ")[1]
    if token != "metlife_director_2026":
        raise ForbiddenAccessError("Forbidden. Access restricted to authorized stadium staff.")
    return token
...
async def get_ops_summary(
    ...
    local_incidents = [inc for inc in active_incidents if inc.get("stadium_id", "stadium_metlife") == stadium_id]
    ...
    try:
        ...
        alert_msgs = [f"{f['type']} {f['name']} approaching capacity in {f.get('predicted_minutes_to_capacity') or f.get('predicted_minutes_to_full')} mins." for f in predictive_forecasts[:2]]
...
async def report_incident(
    ...
    logger.info(f"Incident reported securely: {report.incident_type} at {report.location_node_id}")
